...
final class CRUDSafetyAndRoomPersistenceTests: XCTestCase {
    func testBlockedDeletesForReferencedEntities() throws {
        ...
        let property = try XCTUnwrap(
            try repo.properties(for: project.clientId).first(where: { $0.id == project.propertyId })
        )
        let client = try XCTUnwrap(try repo.clients().first(where: { $0.id == project.clientId }))
