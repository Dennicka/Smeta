...
| D-012 | Calculation contains hardcoded magic percentages for transport/equipment/waste/margin/moms instead of settings/rules. | P0 | PARTIAL | Основные проценты totals path (`transport/equipment/waste/margin/moms`) вынесены в persisted `calculation_rules` и читаются через `CalculationRules`; runtime evidence есть (`EVIDENCE/D012_CALCULATION_RULES.md`). Для `RESOLVED` нужно убрать оставшиеся money-impacting hardcoded coefficients/percent fallbacks (`0.01/0.1/0.2`, VAT fallback `0.25`) в смежном расчётном пути. |
