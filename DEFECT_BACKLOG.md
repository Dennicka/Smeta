...
| D-008 | Document generation relies on hardcoded/demo lines in views instead of real project/estimate/document mapping. | P0 | PARTIAL | Offert/Faktura переведены на repository-backed mapping + verification pass выполнен (code-level + сценарный builder verification). Остаток: Avtal/Kreditfaktura/ÄTA/Påminnelse перевести на тот же подход и подтвердить runtime evidence. |
| D-009 | Document snapshot is incomplete and does not store full immutable document content. | P0 | PARTIAL | Full immutable snapshot и атомарный flow `finalizeDocumentWithSnapshot` внедрены в коде, старый опасный finalize path удалён, добавлены builder/repository verification scripts; однако repository-level runtime verification в текущем Linux окружении пока не подтверждён из-за D-004 (`no such module 'SQLite3'`). |
