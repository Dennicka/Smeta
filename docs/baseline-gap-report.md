...
| backup / restore | PARTIAL | Вынесен testable core-flow backup/restore вне UI-диалогов, restore сделан rollback-safe с проверкой валидности backup и очисткой SQLite sidecar-артефактов. | `Sources/SmetaApp/Services/BackupService.swift`, `Sources/SmetaApp/Data/SQLiteDatabase.swift`, `Sources/SmetaApp/Views/SettingsView.swift` | Добавлен contour-файл `Tests/SmetaAppStartupTests/BackupRestoreContourTests.swift` (A–E сценарии backup/restore). | Runtime smoke по backup/restore отсутствует; macOS-only startup tests в текущем Linux-контуре не исполняются. | До прогона startup contour на Darwin baseline-пункт нельзя поднять до DONE. |
